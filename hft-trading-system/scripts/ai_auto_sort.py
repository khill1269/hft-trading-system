import os
import shutil
import logging

logging.basicConfig(level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s")

# Define categories and keywords to match file types
CATEGORIES = {
    "market_data": ["data", "market", "scraper", "fetch", "api", "ibkr"],
    "trade_execution": ["execution", "order", "trade", "route", "ibkr"],
    "risk_management": ["risk", "hedging", "monitor", "margin"],
    "ai_models": ["ai", "ml", "nn", "rl", "model", "predict"],
    "utils": ["helper", "util", "common", "tools"],
    "config": ["config", "setup", "settings"],
    "hardware_acceleration": ["fpga", "hft", "hardware", "latency"],
    "monitoring": ["alert", "monitor", "logging", "metrics"],
    "core": ["system", "core", "infrastructure", "architecture"],
    "docs": ["overview", "workflow", "documentation", "md", "guide"],
    "ui": ["tsx", "dashboard", "interface", "frontend"],
    "tests": ["test", "testing"],
    "scripts": ["script", "setup", "install", "deploy"],
    "logs": ["log", "logs"]
}

# Define destination directories
DESTINATION_DIRS = {
    "market_data": "src/market_data/",
    "trade_execution": "src/trade_execution/",
    "risk_management": "src/risk_management/",
    "ai_models": "src/ai_models/",
    "utils": "src/utils/",
    "config": "config/",
    "hardware_acceleration": "src/hardware_acceleration/",
    "monitoring": "src/monitoring/",
    "core": "src/core/",
    "docs": "docs/",
    "ui": "ui/",
    "tests": "tests/",
    "scripts": "scripts/",
    "logs": "logs/"
}

# Ensure destination directories exist
for directory in DESTINATION_DIRS.values():
    os.makedirs(directory, exist_ok=True)

SOURCE_DIR = "."

def categorize_and_move_files():
    """Sorts and moves files from the main directory into structured subdirectories."""
    moved_files = 0
    uncategorized_files = []

    for file in os.listdir(SOURCE_DIR):
        file_path = os.path.join(SOURCE_DIR, file)

        # Skip directories and this script itself
        if not os.path.isfile(file_path) or file == "ai_auto_sort.py":
            continue  

        # Categorize the file
        category_found = False
        for category, keywords in CATEGORIES.items():
            if any(keyword in file.lower() for keyword in keywords):
                dest_dir = DESTINATION_DIRS[category]
                os.makedirs(dest_dir, exist_ok=True)
                shutil.move(file_path, os.path.join(dest_dir, file))
                logging.info(f"Moved {file} to {dest_dir}")
                category_found = True
                moved_files += 1
                break

        if not category_found:
            uncategorized_files.append(file)

    if uncategorized_files:
        logging.warning(f"Could not categorize {len(uncategorized_files)} files, leaving them in the main directory.")
        for file in uncategorized_files:
            logging.warning(f"Uncategorized: {file}")
    else:
        logging.info(f"All {moved_files} files successfully sorted!")

if __name__ == "__main__":
    categorize_and_move_files()

